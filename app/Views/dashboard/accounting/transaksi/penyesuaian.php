              ...
              <div class="card-header">
                ...
                <div class="row">
                  <div class="col-md-12 d-none mt-3" id="input-section">
                    ...
                    <div class="row gx-3 mb-3 col-md-12">
                      <label class="small mb-1" for="nilai">
                      ...
                      <input type="hidden" class="nilai-num" value="">

                      <div class="invalid-feedback"></div>
                    </div>
                    <div class="row gx-3 mb-3 col-md-12">
                      <label class="small mb-1" for="waktu">
                        Waktu disesuaikan
                      </label>
                    ...
                    <div class="row gx-3 mb-3 col-md-12">
                      <label class="small mb-1" for="jumlah">
                      ...
                      <input type="hidden" class="jumlah-num" value="">
                ...
                <table id="table-data" class="table table-bordered table-hover">
                  <thead>
                    <tr>
                      ...
                      <th scope="row" class="align-middle text-center">Nilai</th>
                      <th scope="row" class="align-middle text-center">Waktu</th>
                      <th scope="row" class="align-middle text-center">Jumlah</th>
...
<script>
    ...
    $(function () {
      ...
      fetchCategory(data => processCategory( data));
        ...
        btnBatal.on('click', function(){
          ...
          $deskripsi.val('');
          $nilai.val('Rp 0');
          $nilai.siblings('.nilai-num').val('');
          $waktu.val('');
          $jumlah.val('Rp 0');
          $jumlah.siblings('.jumlah-num').val('');
          $tableForm.empty();
          toggleHide();
        });
        ...
        btnSimpan.on('click', () => {
          ...
          formData.append('nilai', $nilai.siblings('.nilai-num').val());
        ...
        $nilai.on('input',function () {
          ...
          var sanitized = $nilai.val().replace(/[^0-9-]/g, '');
          $nilai.addClass('is-invalid');
          if (sanitized !== '') {
              $nilai.removeClass('is-invalid');
              var number = parseInt(sanitized, 10);
              $nilai.val(formatRupiah(number));
              $nilai.siblings('.nilai-num').val(number);
              updateJumlah();
          }
        });
        $waktu.on('input',function () {updateJumlah()});
        ...
        $tableData.on('click', 'button.btn-edit, button.btn-delete', function () {
            ...
            if (clickedButton.hasClass('btn-edit')) {
                ...
                $nilai.val(formatRupiah(dataEdit.nilai));
                $nilai.siblings('.nilai-num').val(dataEdit.nilai);
                $waktu.val(dataEdit.waktu);
                $jumlah.val(formatRupiah(dataEdit.jumlah));
                $jumlah.siblings('.jumlah-num').val(dataEdit.jumlah);
                $nilai.focus();

                var dataEditDetail = dataEdit.detail;
                for (let i = 0; i < dataEditDetail.length; i++) {
                  
                  let option = jsonCategory.map(e => `<option value='${e.kode}' ${e.kode == dataEditDetail[i].category3_kode ? "selected='selected'" : ''}>${e.kode+'|'+e.name}</option>`).join('');
  
                  let row = `
                      <tr data-status='save'>
                          <td class="align-middle text-center">
                              <div class="category-html">${dataEditDetail[i].category3_name}</div>
              ...
              btnBatal.click();
              $.ajax({
                  url: `<?= url_to('accounting.transaksi.penyesuaian') ?>/delete/${currentID}`,
                  type: "POST",
                  data: {
                    '_method': 'DELETE',
                  },
    ...
    function updateJumlah() {
      if ($nilai.siblings('.nilai-num').val() !== '' && $nilai.siblings('.nilai-num').val() !== 0 && $waktu.val() !== '' && $waktu.val() !== 0) {
        var jumlah = parseInt($nilai.siblings('.nilai-num').val()) / parseInt($waktu.val());
        $jumlah.val(formatRupiah(jumlah));
        $jumlah.siblings('.jumlah-num').val(jumlah);
      } else {
        $jumlah.val(formatRupiah(0));
        $jumlah.siblings('.jumlah-num').val(0);
      }
    }
  ...
  function fetchData(callback) {
      $.ajax({
          url: '<?= url_to('accounting.transaksi.penyesuaian.json') ?>',
          type: 'POST',
          dataType: 'JSON',
          success: function (response) {
              callback(response);
  ...
  function processData(response) {
      for (let i = 0; i < response.length; i++) {
          let detail = response[i].detail.map(e => ({
              category3_kode: e.category3_kode,
              category3_name: e.category3_name,
          ...
          jsonData.push({
            ...
            nilai: parseInt(response[i].nilai),
            waktu: parseInt(response[i].waktu),
            jumlah: parseInt(response[i].jumlah),
            deskripsi: response[i].deskripsi !== null && response[i].deskripsi !== undefined ? response[i].deskripsi : '',
  ...
  function generateData() {
    ...
    if (data.length === 0) {
        $tableData.append('<tr><td colspan="9" class="text-center">Belum ada Transaksi</td></tr>');
        return;
    }
    ...
    for (let i = 0; i < data.length; i++) {
        ...
        let dataDebit = debit.map(e => `
            <p>${e.category3_name}(${formatRupiah(e.debit)})</p>
        `).join('');
        
        let dataKredit = kredit.map(e => `
            <p>${e.category3_name}(${formatRupiah(e.kredit)})</p>
        `).join('');
        ...
        let row = `
            <tr>
                ...
                <td class="align-middle text-center">${data[i].deskripsi}</td>
                <td class="align-middle text-center">${formatRupiah(data[i].nilai)}</td>
                <td class="align-middle text-center">${data[i].waktu}</td>
                <td class="align-middle text-center">${formatRupiah(data[i].jumlah)}</td>
  ...
  function processCategory(response) {
    for (let i = 0; i < response.length; i++) {
      let data = {
        ...
        detail: response[i].detail !== null && response[i].detail !== undefined ? response[i].detail : '',
      };
